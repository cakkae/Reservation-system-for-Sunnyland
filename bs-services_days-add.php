...
if ($_SESSION["logged_in"] != true) {
    ...
    if($action=='del_schedule'){
        ...
        $result = mysql_query($sql) or die("oopsy, error occured when tryin to delete schedule.<br>");
    ...
    if (isset($_GET['delImg']) && $_GET['delImg'] == 'yes' && !empty($_GET['id'])) {
        ...
        @unlink($_SERVER['DOCUMENT_ROOT'] . $baseDir . mysql_result($result, 'img'));
    ...
    if (!empty($_REQUEST["edit_page"]) && $_REQUEST["edit_page"] == "yes") {
        if ($deposit > 0 && $deposit <= 100) {
            ...
            $sql = "UPDATE bs_services SET
                    ...
                    WHERE id='" . $id . "'";
            $result = mysql_query($sql) or die("oopsy, error occured when tryin to update service." . mysql_error() . " - $sql");
            ...
            $query = "UPDATE `bs_service_days_settings` SET
                    ...
                    `showPrice`=".$showPrice."
                    WHERE `idService` = '{$id}'";
            $result = mysql_query($query) or die("oopsy, error occured when tryin to update service." . mysql_error() . " - $query");
    ...
    <div id="content">
        ...
        <div class="content_block">
            ...
            <h2><?php if (!empty($id)) { echo  TXT_DAY_SRV_EDIT;}else{echo TXT_DAY_SRV_ADD;}?>
            ...
            <form action="bs-services_days-add.php" enctype="multipart/form-data" method="post" name="ff1">
                ...
                <div class="form-row">
                    ...
                    <div class="cell medium3">
                        ...
                        <div class="valign">
                            <input type="radio" name="showPrice" id="showPrice"
                                   value="1" <?php echo $showPrice == "1" ? "checked" : "" ?>/> <?php echo YES; ?>
                            &nbsp;
                            <input type="radio" name="showPrice" id="showPrice"
                                   value="0" <?php echo $showPrice == "0" ? "checked" : "" ?> /> <?php echo NO; ?>
                            &nbsp;
                        </div>
