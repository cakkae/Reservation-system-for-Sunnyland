                ...
                if ($datetocheck < date("Y-m-d")) {
                    ...
                    if (count($events) > 0) {
                        ...
                        foreach ($events as $evt) {
                            $row = $evt['event'];
                            $spaces_left = $evt['qty'];
                            $click = getOption('use_popup') ? "getLightbox2('" . $row['id'] . "'," . $serviceID . ",'".date("Y-m-d",strtotime($row['eventDate']))."');" :
                             "window.location.href='event-booking.php?eventID=" . urlencode($row['id']) . "&serviceID=" . $serviceID . "&date=".date("Y-m-d",strtotime($row['eventDate']))."'";
                    ...
                    if(getServiceSettings($serviceID, 'type')=='t'){
                        ...
                        if ($cur_spots > 0) {
                            $bgClass = "cal_reg_on";
                            $clickTime = getOption('use_popup') ? "getLightbox('" . $datetocheck . "'," . $serviceID . ");" : "window.location.href='booking.php?date=" . urlencode($datetocheck) . "&serviceID=" . $serviceID . "'";
...
function checkSpotsForTimeInterval($serviceID,$from,$to,$qty,$id){
    ...
    //print $sql;
    $res = mysql_query($sql);
    $reserved = mysql_result($res,0,'sum');
...
function get_payment_info($orderID) {
    ...
    if (empty($bookingInfo['eventID'])) {
        ...
        if($serviceSettings['type']=='t'){
            ...
            $sSQL = "SELECT * FROM bs_reservations_items WHERE reservationID='" . $orderID . "' ORDER BY reserveDateFrom ASC";
            $result = mysql_query($sSQL) or die("err: " . mysql_error() . $sSQL);
            $orderInfo = mysql_fetch_assoc($result);
...
function sendPaymentEmails($orderId,$by=""){
    ...
    if (!empty($row['eventID'])) {
       ...
       sendMail(getAdminMail(), $subject, "paymentConfirmationEvent.php",$row['sid'], $data);
        ...
        $ress = mysql_query($ssql);
        if($serviceSettings['type']=='t'){
            $data['isTime']=true;
            $time=array();
            $date="";
            while($r = mysql_fetch_assoc($ress)){
           ...
           $data['isDay']=true; 
           $bookData = mysql_fetch_assoc($ress);
