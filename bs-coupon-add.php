    ...
    if (!empty($_REQUEST["edit_page"]) && $_REQUEST["edit_page"] == "yes") {
        ...
        if (empty($id)) {
            if(checkCouponCode($code)){
                ...
                $result = mysql_query($sql) or die("oopsy, error occured when tryin to create new coupon.");
                $id = mysql_insert_id();
        ...
        if (!empty($id)) {
            ...
            $query = "UPDATE `bs_coupons` SET
                    ...
                    `code`='{$code}'
                    WHERE `id` = '{$id}'";
            $result = mysql_query($query) or die("oopsy, error occured when tryin to update coupon." . mysql_error() . " - $query");
