 ...
 * #******************************************************************************/
session_start();
if(!isset($_SESSION['logged_in'])){die('no no no..');}
...
function sanitizeArrForSQL($stringWithQuoutes){
	$arr = explode(',', $stringWithQuoutes);$newArr = array();
	foreach($arr as $k=>$v){if(is_numeric($v)){$newArr[]=$v;}}
	return implode(',',$newArr);
}
function getAllEvents(){
	
	$select_events = " SELECT id, title FROM bs_events";
	$select_result = mysql_query($select_events);
	$events = array();
	while($row = mysql_fetch_assoc($select_result)){$events[]=$row['id'];}
...
$result = mysql_query($SQL) or die("SQL:<br />".$SQL."<br />ERROR:".mysql_error());
